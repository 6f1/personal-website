...
const CleanImage : React.FC<{ source : string, size? : { x : number, y : number }, blur? : boolean}> = ({ source, size, blur }) => {
    ...
    return (
        <div className="clean-image-container">
            ...
            <img className="clean-image-img" src={source} onLoad={() => setShouldDisplay(true)} onError={() => setShouldDisplay(false)} style={{ display : (shouldDisplay) ? "hidden" : undefined, filter : (blur ? "blur(4px)" : undefined) }}/>
        </div>
    );
};
